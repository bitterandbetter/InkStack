...
if [[ -f "$TAURI_CONFIG" ]]; then
  ...
  if grep -q '"targets": \["dmg"\]' "$TAURI_CONFIG"; then
    echo "PASS Tauri bundle target is dmg"
  else
    echo "FAIL Tauri bundle target should be dmg" >&2
    exit 1
  fi
...
if [[ ! -d "$APP_PATH" ]]; then
  echo "WARN standalone app bundle not found: $APP_PATH"
  echo "WARN app bundle checks are skipped because DMG is the release artifact."
  skip_app_checks=1
else
  skip_app_checks=0
fi

if [[ "$skip_app_checks" -eq 0 ]]; then
