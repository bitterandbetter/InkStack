...
- 编辑 / 分屏 / 阅读 / 代码四种原有视图保持不变，并新增所见即所得模式：在同一份 Markdown 源码上隐藏非活动标记、直接排版与编辑（`Cmd/Ctrl+5`）；分屏模式继续支持编辑器与预览滚动联动。
- 所见即所得模式支持标题与行内格式、列表和任务框、链接操作、图片替换、代码/公式/Mermaid 卡片、表格单元格与行列编辑，以及 HTML/脚注/未知扩展的安全源码回退；可在“编辑设置”中关闭实验入口。
